...
def sample_callable_bed(bam_file, ref_file, data):
    ...
    """
    CovInfo = collections.namedtuple("CovInfo", "callable, avg_coverage, depth, raw_callable")
    config = data["config"]
    out_file = "%s-callable_sample.bed" % os.path.splitext(bam_file)[0]
    with shared.bedtools_tmpdir({"config": config}):
        depth_bed, callable_bed, variant_regions_avg_cov = coverage.calculate(bam_file, data)
    ...
    return CovInfo(out_file, variant_regions_avg_cov, depth_bed, callable_bed)
