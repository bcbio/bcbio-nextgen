...
galaxy_location_file = "bwamem2_index.loc"
        ...
        with tx_tmpdir(data) as work_dir:
            with postalign.tobam_cl(data, out_file, bam.is_paired(in_bam)) as (tobam_cl, tx_out_file):
                if not hla_on(data) or needs_separate_hla(data):
                    bwamem2_cmd = _get_bwa_mem2_cmd(data, out_file, ref_file, "-", with_hla=False)
                else:
                    bwamem2_cmd = _get_bwa_mem2_cmd(data, out_file, ref_file, "-", with_hla=True)
                ...
                do.run(cmd, "bwa-mem2 alignment from BAM: %s" % names["sample"], None,
    ...
    if needs_separate_hla(data) and not utils.file_exists(hla_file):
        with tx_tmpdir(data) as work_dir:
            with postalign.tobam_cl(data, hla_file, bam.is_paired(in_bam)) as (tobam_cl, tx_out_file):
                bwamem2_cmd = _get_bwa_mem2_cmd(data, hla_file, ref_file, "-", with_hla=True)
                tx_out_prefix = os.path.splitext(tx_out_file)[0]
                prefix1 = "%s-in1" % tx_out_prefix
                cmd = ("unset JAVA_HOME && "
                       "{samtools} sort -n -l 1 -@ {num_cores} -m {max_mem} {in_bam} -T {prefix1} "
...
def _get_bwa_mem2_cmd(data, out_file, ref_file, fastq1, fastq2="", with_hla=False):
    """Perform piped bwa-mem2 mapping potentially with alternative alleles in GRCh38 + HLA typing.
    ...
    if dd.get_aligner(data) == "sentieon-bwa":
        ...
        bwamem2_exe = "bwa-mem2"
        exports = ""
    bwa = config_utils.get_program(bwa_exe, data["config"])
    num_cores = data["config"]["algorithm"].get("num_cores", 1)
    bwamem2_resources = config_utils.get_resources("bwa-mem2", data["config"])
    bwamem2_params = (" ".join([str(x) for x in bwamem2_resources.get("options", [])])
                  if "options" in bwamem2_resources else "")
    ...
    mem_usage = "-c 250"
    bwamem2_cmd = ("{exports}{bwa} mem {pairing} {c_tags} {mem_usage} -M -t {num_cores} {bwamem2_params} -R '{rg_info}' "
               "-v 1 {ref_file} {fastq1} {fastq2} ")
    return (bwamem2_cmd + alt_cmd).format(**locals())
...
def _can_use_mem(fastq_file, data, read_min_size=None):
    """bwa-mem2 handle longer (> 70bp) reads with improved piping.
...
def align_pipe(fastq_file, pair_file, ref_file, names, align_dir, data):
    ...
    if not utils.file_exists(out_file) and (final_file is None or not utils.file_exists(final_file)):
        ...
        if ("bwa-mem2" not in dd.get_tools_on(data) and
              ("bwa-mem2" in dd.get_tools_off(data) or not _can_use_mem(fastq_file, data, min_size))):
...
def _align_mem(fastq_file, pair_file, ref_file, out_file, names, rg_info, data):
    """Perform bwa-mem2 alignment on supported read lengths.
    """
    with postalign.tobam_cl(data, out_file, pair_file != "") as (tobam_cl, tx_out_file):
        cmd = ("unset JAVA_HOME && "
            "%s | %s" % (_get_bwa_mem2_cmd(data, out_file, ref_file, fastq_file, pair_file, with_hla=False), tobam_cl))
        do.run(cmd, "bwa-mem2 alignment from fastq: %s" % names["sample"], None,
...
def _align_mem_hla(fastq_file, pair_file, ref_file, out_file, names, rg_info, data):
    """Perform bwa-mem2 alignment on supported read lengths with HLA alignments
    """
    with postalign.tobam_cl(data, out_file, pair_file != "") as (tobam_cl, tx_out_file):
        cmd = ("unset JAVA_HOME && "
            "%s | %s" % (_get_bwa_mem2_cmd(data, out_file, ref_file, fastq_file, pair_file, with_hla=True), tobam_cl))
        do.run(cmd, "bwa-mem2 alignment from fastq: %s" % names["sample"], None,
                [do.file_nonempty(tx_out_file), do.file_reasonable_size(tx_out_file, fastq_file)])
        return out_file
...
def build_bwamem2_index(fasta_file, data):
    bwa = config_utils.get_program("bwa-mem2", data["config"])
...
def filter_multimappers(align_file, data):
    ...
    https://bioinformatics.stackexchange.com/questions/508/obtaining-uniquely-mapped-reads-from-bwa-mem2-alignment
    """
